...
The provisioner creates active consumer `shorts_factory_backend`, a hashed active API credential and active `quiz_delivery` entitlement scoped to `A2` / `T05` approved/published delivery. The raw API key is written only to the gitignored env file and must be handed over through a private channel, not GitHub.
...
curl -sS "${QUIZ_BANK_API_BASE_URL}/v1/quiz-items/next" \
  ...
  --data '{"consumer_id":"shorts_factory_backend","cefr_level":"A2","theme_ids":["T05"]}'
```
...
Requests outside `A2` / `T05` must return a controlled denial or no eligible item, never another scoped item.
