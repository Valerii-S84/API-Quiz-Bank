...
POSTGRESQL_CLAIM_ITEM_SQL_TEMPLATE = """
        ...
        candidate AS (
            ...
            JOIN selection_queue_items sqi ON sqi.queue_item_id = qc.queue_item_id
            JOIN selection_queues sq ON sq.queue_id = qc.queue_id
            JOIN quiz_items qi ON qi.item_id = sqi.item_id
            WHERE sq.queue_status IN ('ready', 'draining')
              AND sqi.claim_status = 'available'
