...
| OPS-PROD-013 production corpus volume gate exists | blocked-external | `tools/quizbank_production_corpus_gate.py`, `reports/publication/production_corpus_gate_2026-05-10.json`, `tests/test_production_corpus_gate.py`, existing draft/blocked/retired runtime negative-control tests | Gate artifact is closed, but production corpus volume is `NO-GO`: 30,974 active rows, 0 approved/published deliverable production items. |
...
deployment, paid launch, production content volume, remote release-governance
enforcement, real scale or external legal-advice claims without separate
