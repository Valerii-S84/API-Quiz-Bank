...
| OPS-PROD-002 monitored backups exist | closed-protected-production | `reports/roadmap/production_postgresql_runtime_closure_2026-05-10.md`, `api-quiz-bank-postgres-backup.timer enabled active`, backup service `success/0`, `scripts/api_quiz_bank_postgres_backup.sh`, backup runbooks, database operations tests | Closed for PostgreSQL runtime backups with repository-side offsite/managed backup, retention, encryption and RPO/RTO controls; runtime offsite/managed evidence must be recorded separately when configured. |
...
| OPS-PROD-013 production corpus volume gate exists | closed-protected-production | `tools/quizbank_production_corpus_gate.py`, `tools/promote_verified_corpus_to_production.py`, `tools/run_production_corpus_postgresql_smoke.py`, `reports/publication/owner_corpus_approval_2026-05-11.json`, `reports/publication/verified_corpus_promotion_2026-05-11.json`, `reports/imports/production_corpus_postgresql_smoke_2026-05-11.json`, `reports/publication/production_corpus_gate_2026-05-11.json`, `tests/test_production_corpus_gate.py`, existing draft/blocked/retired runtime negative-control tests | Closed for owner-approved active corpus: 30,974 active rows, 30,974 published deliverable production items. |
| OPS-PROD-014 legal/privacy gate exists | closed-protected-production | `reports/compliance/legal_privacy_gate_2026-05-10.json`, `reports/compliance/legal_review_record.md`, `policies/privacy_notice_baseline.md`, `policies/data_retention_policy.md`, `runbooks/privacy_request_workflow.md`, `tests/test_legal_privacy_gate.py` | Closed only for owner-operated protected runtime; exact public legal entity/jurisdiction and broad/school/paid launch remain pending. |
| OPS-PROD-015 release governance controls exist | partial | `.github/branch_protection_main.json`, `.github/workflows/ci.yml`, `.github/pull_request_template.md`, `CHANGELOG.md`, `runbooks/migration_approval_checklist.md`, `reports/release/release_governance_gate_2026-05-10.json`, `tests/test_release_governance_gate.py` | Repository-side controls exist; actual GitHub branch protection and release tag execution must be verified remotely. |
| OPS-PROD-016 scale/load smoke exists | partial | `tools/run_mvp_load_smoke.py`, `reports/scale/protected_runtime_load_smoke_2026-05-10.json`, Docker Compose CPU/memory limits, `tests/test_scale_load_gate.py` | Local 8-concurrent TestClient smoke exists; external production traffic/load test remains required before real scale claim. |
...
deployment, paid launch, remote release-governance enforcement, real scale or external legal-advice claims without separate
