...
| Scale/load | Smoke threshold remains closed for the earlier owner-operated protected business-route check: documented VPS access `root@valerchik.de` was used, isolated test consumer/key `load-smoke-test-2026-06-11` was created in the PostgreSQL runtime through the API container path, and `reports/scale/protected_business_route_load_smoke_2026-06-11.json` records 60/60 protected `/v1/quiz-items/next` responses with zero 5xx/timeouts/errors. Full staged load remains open: `reports/scale/full_load_test_2026-06-11.json` is `Partial`; `reports/scale/load_bottleneck_diagnostics_2026-06-11.json` confirms the bottleneck as the unfiltered candidate selection path fetching/scoring about 30k rows with correlated delivery-history subqueries. Local remediation evidence is recorded in `reports/scale/next_route_remediation_2026-06-11.md`, `reports/scale/next_route_explain_before_after_2026-06-11.md` and `reports/scale/next_route_perf_after_fix_2026-06-11.json`. PostgreSQL pre-deploy validation is recorded in `reports/scale/next_route_postgres_validation_2026-06-11.md`; clean source-of-truth deploy path preparation is recorded in `reports/scale/next_route_deploy_path_gate_2026-06-11.md`, `reports/scale/server_dirty_worktree_review_2026-06-11.md` and `reports/scale/next_route_deploy_plan_2026-06-11.md`. The production hotfix deploy was executed on 2026-06-12 after safe dirty worktree backup/stash; migration `011` was applied; API-only rebuild/restart completed; and `reports/scale/next_route_postfix_smoke_2026-06-12.json` records 85/85 protected `/v1/quiz-items/next` responses with zero 5xx/timeouts, p95 211.445 ms, p99 241.626 ms, max candidate count 300 and max sampled Postgres CPU 8.36%. Required external evidence before closing full scale: a separate protected staged load test. This is not broad public scale, paid launch, school launch, support/SLA or legal/privacy approval. |
