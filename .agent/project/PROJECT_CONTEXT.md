...
|---|---|---|---|
| `Telegram Bot API` | Planned quiz/poll delivery consumer | Token secret is wired on the VPS as a root-only file and mounted by file path; one controlled direct Bot API send succeeded; local worker supports dry-run and explicitly approved real-send mode | Telegram is an adapter/consumer and must not own selection logic; deployed worker real-send execution is not connected/proven on the VPS pilot runtime yet |
...
| `VPS Docker runtime` | API Quiz Bank pilot runtime with protected public route | Configured at `/opt/api-quiz-bank` with local SQLite data under service `var/api-quiz-bank/`, backups in `/var/backups/api-quiz-bank`, API bound to `127.0.0.1:8010`, and Caddy route `api.valerchik.de` requiring `X-API-Key` | Deployed worker real-send path is not proven; this is not a production deployment; public route is smoke-proven but public beta remains gated by support/legal/monitoring evidence |
...
- Deploy / production boundaries: `VPS Docker deploy is configured and running for the pilot runtime with protected Caddy route api.valerchik.de using X-API-Key; local Telegram worker code exists, but deployed worker real-send evidence is not recorded; any future weakening of the API-key gate, Telegram delivery on external targets, runtime secret wiring beyond this service, production database operation or production claim is outside normal safe edit scope unless explicitly requested`
