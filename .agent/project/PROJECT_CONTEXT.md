...
|---|---|---|---|
| `Telegram Bot API` | Planned quiz/poll delivery consumer | Planned external integration; not configured in current snapshot | Telegram is an adapter/consumer and must not own selection logic; no bot integration is connected to the VPS pilot runtime |
...
| `VPS Docker runtime` | Isolated API Quiz Bank pilot runtime | Configured at `/opt/api-quiz-bank` with data in `/var/lib/api-quiz-bank`, backups in `/var/backups/api-quiz-bank`, and API bound to `127.0.0.1:8010` | Public domain/Caddy integration is not configured; bot integration is not configured; this is not a production/public deployment |
...
- Deploy / production boundaries: `VPS local-only Docker deploy is configured and running for the pilot runtime; public domain/Caddy integration is not configured; bot integration is not configured; any future public access, Caddy routing, Telegram delivery, runtime secret wiring beyond this service, production database operation or production claim is outside normal safe edit scope unless explicitly requested`
...
- Restricted hosts / environments: `The existing VPS runtime must be changed only through explicit deploy scope; any future production/staging database, Telegram Bot API environment, billing provider, public Caddy route, external client environment or unmanaged host outside the local workspace remains restricted`
