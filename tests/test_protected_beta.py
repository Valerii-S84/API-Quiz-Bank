...
from quizbank_mvp.telegram_delivery import TelegramSendResult  # noqa: E402
...
class ProtectedBetaTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_channel_seed_and_schedule_are_configured(self) -> None:
        ...
        self.assertEqual(
            ...
            [("08:22", "A2", "T11", 3), ("12:47", "B1", "T03", 3), ("20:15", "B2", "T10", 3)],
        )
    ...
    def seed_slot_items(self, level: str, theme_id: str, count: int) -> None:
        ...
        rows = [json.dumps(self.slot_item(base_item, level, theme_id, index), ensure_ascii=False)
                for index in range(1, count + 1)]
        fixture_path = Path(self.temp_directory.name) / f"{level}_{theme_id}.jsonl"
        fixture_path.write_text("\n".join(rows), encoding="utf-8")
        seed_control_fixture(self.db_path, fixture_path, "approved", source_id=f"src_{level}_{theme_id}")
